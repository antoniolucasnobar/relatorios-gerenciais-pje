-- [T989][agrupado por classe]
-- EXPLAIN ANALYZE
    select
        cj.ds_classe_judicial AS "Classe"
        , COUNT (cj.ds_classe_judicial) AS "Quantidade"
        , '$URL/execucao/T991?CLASSE_PROCESSUAL='||cj.id_classe_judicial
        as "Ver Processos"
...
--                                                 AND assunto.id_assunto_trf = pass.id_assunto_trf)
    ...
    where p.id_agrupamento_fase != 5
      ...
      AND pt.id_tarefa = COALESCE(:TAREFA, pt.id_tarefa)
        ...
        GROUP BY cj.id_classe_judicial, cj.ds_classe_judicial
order by cj.ds_classe_judicial
